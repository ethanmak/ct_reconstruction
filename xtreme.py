...
class Xtreme(object):
    def __init__(self, file):
        ...
        if self.okay:
            ...
            self.samples = (
                h[7] - skip_samples
            )  # number of sample measurements at each angle
            ...
            self.radius = (float(self.samples) / 2.0) / math.tan(
                float(self.fan_theta) / 2.0
            )
    ...
    def get_rsq_scan(self, angle):
        ...
        for scan in range(0, self.scans):
            ...
            f.seek(
                (self.samples + self.skip_samples) * (self.angles - angle - 1) * 2, 1
            )
    ...
    def fan_to_parallel(self, X):
        ...
        index = yo > atheta
        xo[index] = (
            (xo1[index] - (5.0 * samples / 6.0)) / np.cos(yo[index] - 2.0 * atheta)
            + c
            + samples / 3.0
        )
        index = yo < -atheta
        xo[index] = (
            (xo1[index] - (samples / 6.0)) / np.cos(yo[index] + 2.0 * atheta)
            + c
            - samples / 3.0
        )
