...
def attenuate(original_energy, coeff, depth):
    ...
    if len(coeff) != energies:
        raise ValueError(
            "input coeff has different number of energies to input original_energy"
        )
    ...
    if len(depth) != samples:
        raise ValueError(
            "input depth has different number of samples to input original_energy"
        )

    # Work out residual energy for each depth and at each energy
    for i in range(energies):
        for j in range(samples):
            original_energy[i][j] *= np.exp(-coeff[i] * depth[j])
