...
def phantom(ellipses, n):
    ...
    for ellipse in ellipses:
        ...
        cosp = math.cos(phi)
        sinp = math.sin(phi)
        values = ((x_center * cosp + y_center * sinp) ** 2) / asq + (
            (y_center * cosp - x_center * sinp) ** 2
        ) / bsq
...
def ct_phantom(names, n, type, metal=None):
    ...
    if type == 1:
        ...
        for r in np.arange(n * 0.04, n * 0.4, n * 0.04):
            ...
            for a in angles:
                x[int(round(n / 2 + r * math.cos(a)))][
                    int(round(n / 2 + r * math.sin(a)))
                ] = nmetal
        ...
        if nmetal > tissue:
            if type == 3:
                ...
                m = [[100, 0.1, 0.1, -0.48, -0.01, 0], [100, 0.08, 0.06, 0.48, 0, 0]]
            elif type == 5:
                # sphere with three satellites
                m = [
                    [100, 0.05, 0.05, -0.43, -0.03, 0],
